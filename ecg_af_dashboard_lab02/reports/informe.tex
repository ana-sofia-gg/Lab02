...
\begin{itemize}
    \item \textbf{Ritmo no-FA (Sinusal):} Presenta una distribución de intervalos RR relativamente estable y acotada, con una variabilidad moderada. En este proyecto SDNN, RMSSD y $CV_{RR}$ se interpretan como descriptores de irregularidad ventricular durante el segmento, nunca como una estimación de modulación autonómica.
...
Para ilustrar este comportamiento, la Figura~\ref{fig:rr_hist_04043} muestra el histograma de los intervalos RR generado de forma automatizada para el registro \textbf{04043}, permitiendo visualizar la dispersión y densidad de las duraciones latido a latido características del análisis.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{../figures/rr_hist_04043.png}
    \caption{Histograma de intervalos RR para el registro 04043, evidenciando la distribución de las duraciones latido a latido obtenida mediante el script de reproducción.}
    \label{fig:rr_hist_04043}
\end{figure}

Los descriptores de la tabla siguiente los genera
\texttt{scripts/reproduce.py} en \texttt{results/summary\_table.tex};
ninguna cifra se transcribe a mano.

\IfFileExists{results/summary_table.tex}
  {\input{results/summary_table.tex}}
  {\textit{Ejecute \texttt{uv run python scripts/reproduce.py} para
   generar esta tabla.}}
